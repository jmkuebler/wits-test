...
class FdaFalkon(FalkonBase):
    """Falkon Kernel Fisher Discriminant Analysis solver.

    This estimator object solves approximate kernel Fisher Discriminant Analysis problems with Nystroem
    projections and a fast optimization algorithm as described in [flk_1]_, [flk_2]_, TODO: add ours when available.
    ...
    -----------
    kernel
        Object representing the kernel function used for KFDA.
    penalty : float
    ...
    Examples TODO: Adopt to our case
    --------
    ...
    >>> model = FdaFalkon(kernel=kernel, penalty=1e-6, M=500, options=options)
    >>> model.fit(X, Y)
    ...
    def fit(self,
        ...
        with TicToc("Calcuating Preconditioner of size %d" % self.M, debug=self.options.debug):
            ...
            precond = falkon.preconditioner.FdaPreconditioner(self.penalty, self.kernel, pc_opt)
            # TODO: ny_points also contains the labels. We need to adopt the init. so far it is the old precond
        ...
        # TODO: currently we force to precompute the kernel matrix! (by setting store threshold to zero)
        if self._can_store_knm(X, ny_points[0], available_ram, store_threshold=0):
            Knm = self.kernel(X, ny_points[0], opt=self.options)
        else:
            Knm = None
        ...
        # TODO: Check what we actually want to use as validation error
        ...
        with TicToc('Computing Falkon iterations', debug=self.options.debug):
            ...
            optim = falkon.optim.FdaConjugateGradient(self.kernel, precond, o_opt)
            if Knm is not None:
                # TODO think about using MMD as initial solution
                beta = optim.solve(
                    Knm, None, ny_points[1], Y, self.penalty, initial_solution=None,
                    max_iter=self.maxiter, callback=validation_cback)
            else:
                # TODO: This case needs to be implemented
                # TODO: think about MMD as initial solution
                beta = optim.solve(
