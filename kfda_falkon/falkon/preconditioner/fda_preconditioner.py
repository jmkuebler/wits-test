...
class FdaPreconditioner(Preconditioner):
    #TODO: Adopt eveything
    ...
    def init(self, X: Union[torch.Tensor, SparseTensor], Y):
        ...
        Y : torch.Tensor
            The (M x 1) matrix of class labels for the Nystroem centers
        """
        dtype = X.dtype
        dev = X.device
        ...
        if self._use_cuda:
            ...
            with TicToc("LAUUM", debug=self.params.debug):
                # Product lower(fC).T @ lower(fC) : lower(fC) = T @ N @ N @ T.T
                C = fda_lauum_wrapper(C, Y, upper=False, use_cuda=self._use_cuda, opt=self.params)


        with TicToc("Cholesky 2", debug=self.params.debug):
            # lower(fC) = 1/M * T @ N @ N @ T.T
            mul_triang(C, upper=False, preserve_diag=False, multiplier=1 / M)
            # lower(fC) = 1/M * T @ N @ N @ T.T + lambda * I
